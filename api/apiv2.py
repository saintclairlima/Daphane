...
from api.rag_chain import RAGChain
...
print(f'Criando RAGChain (usando {environment.MODELO_DE_EMBEDDINGS} - device={environment.DEVICE})...')
ragchain = RAGChain()
...
async def gerar_resposta(dadosRecebidos: DadosChat):
    return StreamingResponse(ragchain.consultar(dadosRecebidos), media_type='text/plain')
